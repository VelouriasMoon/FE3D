...
{
    /// <summary>
    /// Compressor and decompressor for the LZ-0x13 format used in many of the games for the
    /// newer Nintendo consoles and handhelds.
    /// </summary>
    public sealed class LZ13 : NitroCFormat
    {
        /// <summary>
        ...
        /// <summary>
        /// Creates a new instance of the LZ-13 compression format.
        /// </summary>
        public LZ13() : base(0x13) { }
        ...
        /// <summary>
        /// Decompresses the input using the LZ-13 compression scheme.
        /// </summary>
        public override long Decompress(Stream instream, long inLength, Stream outstream)
        {
            ...
            if (type != base.magicByte)
                throw new InvalidDataException("The provided stream is not a valid LZ-0x13 "
                            + "compressed stream (invalid type 0x" + type.ToString("X") + ")");
            byte[] compressedSize = new byte[3];
            instream.Read(compressedSize, 0, 3);
            byte type2 = (byte)instream.ReadByte();
            if (type2 != 0x11)
                throw new InvalidDataException("The provided stream is not a valid LZ-0x13 "
                            + "compressed stream (invalid type 0x" + type.ToString("X") + ")");
            readBytes += 4;
        ...
        public unsafe override int Compress(Stream instream, long inLength, Stream outstream)
        {
            ...
            outstream.WriteByte((byte)0x11);
            outstream.WriteByte((byte)(inLength & 0xFF));
            outstream.WriteByte((byte)((inLength >> 8) & 0xFF));
            outstream.WriteByte((byte)((inLength >> 16) & 0xFF));

            int compressedLength = 8;
            ...
            fixed (byte* instart = &indata[0])
            {
                ...
                while (readBytes < inLength)
                {
                    ...
                    int lead = readBytes - compressedLength;
                    if (maxlead <= lead)
                        maxlead = lead;
                }
                ...
                int lz13 = maxlead + compressedLength;

                outstream.Seek(1, SeekOrigin.Begin);
                outstream.WriteByte((byte)(lz13 & 0xFF));
                outstream.WriteByte((byte)((lz13 >> 8) & 0xFF));
                outstream.WriteByte((byte)((lz13 >> 16) & 0xFF));
            }
